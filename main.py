...
class BikeCommuteDashboard:
        ...
        with ui.header().classes("w-full"):
            with ui.row().classes("w-full items-center justify-between"):
                ...
                with ui.row():
                    ...
                    ui.button("Force Resync (Today)", on_click=self.force_resync_today).props(
                        "color=warning"
                    )
    ...
    def force_resync_today(self):
        """Force resync of files modified today"""
        from datetime import datetime

        today = datetime.now().strftime("%Y-%m-%d")
        print(f"\n🔄 Force resync for files from {today}")
        reset_files = self.drive_manager.reset_processed_files_after_date(today)

        if reset_files:
            ui.notify(f"Reset {len(reset_files)} files. Starting sync...", type="info")
            self.sync_rides()
        else:
            ui.notify(f"No files found from {today} to reset", type="warning")
